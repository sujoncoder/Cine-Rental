...
const CartDetails = ({ onClose }) => {
  ...
  const handleRemoveCart = (e, itemId) => {
    const remainItem = cartData.filter((item) => {
      return item.id !== itemId;
    });

    setCartData([...remainItem]);
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[420px] sm:max-w-[600px] lg:max-w-[790px] p-4 max-h-[90vh] overflow-auto">
        <div className="bg-white shadow-md dark:bg-[#12141D] rounded-2xl overflow-hidden p-5 md:p-9">
          ...
          <div className="space-y-8 lg:space-y-12 max-h-[450px] overflow-auto mb-10 lg:mb-14">
            {cartData.length == 0 ? (
              <p className="text-white text-3xl">The cart is empty</p>
            ) : (
              cartData.map((item) => (
                <div key={item.id} className="grid grid-cols-[1fr_auto] gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="max-md:text-xs text-[#9295aa]">
                        {item.genre}
                      </p>
                      <span className="max-md:text-xs text-white">
                        ${item.price}
                      </span>
                    </div>
                  </div>
